...
- The output may be in Korean or English depending on the source text.
- The important part is to preserve the meaning faithfully.
...
- Prefer the most general applicable concept
- Always return at least 1 topic

Entities:
- Must be proper nouns or formally named constructs
...
translator = ChatOllama(model=MODEL_NAME, temperature=0)
...
def repair_json_text(text: str) -> str:
    ...
    if in_string:
        repaired += '"'
    if stack:
        repaired += "".join(closing_chars[char] for char in reversed(stack))

    return repaired


TRANSLATE_PROMPT = """
Translate the following JSON values into natural Korean.
Rules:
- Keep the JSON structure unchanged.
- Translate only the string values.
- Preserve the meaning and do not add extra explanation.
- If the input is already in Korean, keep it as-is.
...
def translate_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        translated = translator.invoke(
            TRANSLATE_PROMPT.format(payload=json.dumps(payload, ensure_ascii=False, indent=2))
        )
        ...
        return normalize_json_values(payload)


def normalize_json_values(payload: dict[str, Any]) -> dict[str, Any]:
    if not isinstance(payload, dict):
        return payload
    ...
    for key, value in payload.items():
        if isinstance(value, str):
            normalized[key] = normalize_korean_text(value)
        elif isinstance(value, list):
            normalized[key] = [normalize_json_values(item) if isinstance(item, dict) else normalize_korean_text(item) if isinstance(item, str) else item for item in value]
        elif isinstance(value, dict):
            normalized[key] = normalize_json_values(value)
        else:
...
def invoke_json(template: str, **kwargs: Any) -> dict[str, Any]:
    ...
    chain = prompt | llm
    text = chain.invoke(kwargs).content

    if not isinstance(text, str):
        raise TypeError(f"Expected model output to be a string, got {type(text)!r}")
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"Model returned invalid JSON: {cleaned[:1000]}") from exc

    normalized = normalize_json_values(payload)
    return translate_payload(normalized)
